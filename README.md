...
В полях класса хранятся следующие данные:
- _cartItems: HTMLElement - обертка для списка товаров
...
В полях класса хранятся следующие данные:
- _contentContainer: HTMLDivElement - элемент для содержимого модального окна
...
Так же класс предоставляет набор методов для взаимодействия с этими данными.
- сеттеры для полей модалки
- togglePaymentButton(btnName: IOrder['payment']): void - устанавливает правильные состояния для кнопок
